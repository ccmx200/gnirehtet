...
impl Command for DemandCommand {
    fn command(&self) -> &'static str {
        "demand"
    }
    ...
    fn description(&self) -> &'static str {
        "Enable reverse tethering with auto-recovery for one device:\n  \
         - install the client if necessary;\n  \
         - start the client;\n  \
         - start the relay server;\n  \
         - monitor the device connection;\n  \
         - on device disconnect, detect it and restart the client\n    \
           (re-establish adb reverse tunnel and re-launch) when it\n    \
           reconnects;\n  \
         - on Ctrl+C, stop both the relay server and the client."
    }
...
/// Callback for demand mode: monitors a specific device and restarts the client
/// when the device disconnects then reconnects.
struct DemandCallback {
...
impl DemandCallback {
    fn matches(&self, serial: &str) -> bool {
        match &self.serial {
            Some(expected) => expected == serial,
            None => true,
        }
    }
}

impl AdbMonitorCallback for DemandCallback {
    fn on_new_device_connected(&self, serial: &str) {
        if !self.matches(serial) {
            return;
        }
        if self.running.load(Ordering::SeqCst) {
            debug!(target: TAG, "Device {} already running, skip", serial);
            return;
        }
        info!(target: TAG, "Device {} connected, starting client...", serial);
        self.running.store(true, Ordering::SeqCst);
        let dns_servers = self.dns_servers.as_deref();
        let routes = self.routes.as_deref();
        async_start(Some(serial), dns_servers, routes, self.port);
    }
    ...
    fn on_device_disconnected(&self, serial: &str) {
        ...
        info!(
            target: TAG,
            "Device {} disconnected, will restart when reconnected", serial
        );
...
fn cmd_demand(
    ...
    // spawn a thread to monitor device (dis)connection and auto-restart the client
    let monitor_serial = serial.map(String::from);
    let monitor_dns = dns_servers.map(String::from);
    let monitor_routes = routes.map(String::from);
    thread::spawn(move || {
        let callback = DemandCallback {
            serial: monitor_serial,
            dns_servers: monitor_dns,
            routes: monitor_routes,
            port,
            running: Arc::new(AtomicBool::new(true)),
        };
        let mut adb_monitor = AdbMonitor::new(Box::new(callback));
        adb_monitor.monitor();
    });
...
fn cmd_start(
    ...
    let pc_name = get_computer_name();
    let mut adb_args = vec![
        "shell".to_string(),
        "am".to_string(),
        "start".to_string(),
        "-a".to_string(),
        "com.genymobile.gnirehtet.START".to_string(),
        "-n".to_string(),
        "com.genymobile.gnirehtet/.GnirehtetActivity".to_string(),
    ];
    ...
    adb_args.append(&mut vec!["--es".to_string(), "pcName".to_string(), pc_name]);
