...
pub trait AdbMonitorCallback {
    ...
    fn on_device_disconnected(&self, _serial: &str) {}
}
...
impl AdbMonitor {
    ...
    fn handle_packet(&mut self, packet: &str) {
        ...
        for serial in &self.connected_devices {
            if !current_connected_devices.contains(serial) {
                self.callback.on_device_disconnected(serial.as_str());
            }
        }
